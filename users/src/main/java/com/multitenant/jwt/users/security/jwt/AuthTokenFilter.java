package com.multitenant.jwt.users.security.jwt;
...
public class AuthTokenFilter extends OncePerRequestFilter {
	...
	private final String idAuthorities = "authority";
	
	final String TENANT_ID_ATTRIBUTE = JwtUtils.TENANT_ID_ATTRIBUTE;
	final String CLAIM_KEY_AUTHORITIES = JwtUtils.CLAIM_KEY_AUTHORITIES;
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			if (jwt != null && jwtUtils.validateJwtToken(jwt)) {
				...
				String tenantId = jwtUtils.getClaim(jwt, TENANT_ID_ATTRIBUTE, String.class);
				...
				if(tenantId != null) {
					...
					request.setAttribute(TENANT_ID_ATTRIBUTE, "_" + tenantId);
	...
	private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request, String jwt) {
        ...
        if (token != null) {
            // parse the token.
            String user = Jwts.parser().setSigningKey(jwtSecret).parseClaimsJws(jwt).getBody().getSubject();
